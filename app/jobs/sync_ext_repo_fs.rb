...
#
# This makes the same for letters (file system/eXist) as ADL
# (sync_ext_repo_ADL.rb) does using (git/eXist)
# 
...
class SyncExtRepoFS
  ...
  def self.perform(repo_id)
    Resque.logger.debug "Starting FS sync"
    repo = Administration::ExternalRepository[repo_id]
    repo.clear_sync_messages
    repo.add_sync_message("Starting FS sync")
    proccessed_files = 0
    updated_files = 0
    added_files = 0
    new_instances = 0
