module Yggdrasil
  class PreservationManager
    ...
    def update_preservation_profile(params, instance)
      logger.info "Update preservation profile for #{instance.class} - #{instance.id}"
      cascade_preservation(params, instance)
      ...
      if params[:commit] && params[:commit] == Constants::PERFORM_PRESERVATION_BUTTON
        initiate_preservation(instance)
        "Preservation profile for the #{instance.class} successfully updated and the preservation has begun."
      else
        "Preservation profile for the #{instance.class} successfully updated"
      end
    end
    ...
    def cascade_preservation(params, element)
      if element.can_perform_cascading? && params['preservation']['cascade_preservation'] == Constants::CASCADING_EFFECT_TRUE
        element.cascading_elements.each do |pib|
          update_preservation_profile(params, pib)
        end
      end
    end

    # Updates the preservation profile for a given element (e.g. a basic_files, a instance, etc.)
    ...
    def set_preservation_profile(profile, comment, element)
      ...
      if (profile.blank? || element.preservationMetadata.preservation_profile.first == profile) &&
          (comment.blank? || element.preservationMetadata.preservation_comment.first == comment)
